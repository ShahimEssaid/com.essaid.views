...
public enum RequestType {
  ...
  public static RequestType getRequestType(ViewRequest request) {
    Method method = request.getInvokedMethod();
    ...
    if (methodName.startsWith(IS.prefix)) {
      if (method.getParameterCount() != 0 || method.getReturnType() != boolean.class) {
        throw new IllegalStateException(
            "Method signature does not match with the IS " + "pattern. Method: " + method);
      }
      type = IS;
      ...
      if (methodName.endsWith(GET_OR_DEFAULT.suffix)) {
        if (method.getParameterCount() != 1 || method.getReturnType() == Void.class
          ...
          throw new IllegalStateException(
              "Method signature does not match with the GET OR DEFAULT " + "pattern. Method: "
                  + method);
        }
        type = GET_OR_DEFAULT;
        ...
        if (method.getParameterCount() != 1 || method.getReturnType() == Void.class
            || method.getReturnType() != method.getParameterTypes()[0]) {
          throw new IllegalStateException(
              "Method signature does not match with a GET OR SET " + "pattern. Method: " + method);
        }
        type = GET_OR_SET;
      } else if (methodName.endsWith(GET_OR_CREATE.suffix)) {
        if (method.getParameterCount() != 0 || method.getReturnType() == Void.class) {
          throw new IllegalStateException(
              "Method signature does not match with a GET OR CREATE " + "pattern. Method: "
                  + method);
        }
        type = GET_OR_CREATE;
        ...
        if (method.getParameterCount() != 0 || method.getReturnType() == Void.class) {
          throw new IllegalStateException(
              "Method signature does not match with a GET ADD " + "pattern. Method: " + method);
        }
        type = GET_ADD;
      } else {
        if (method.getParameterCount() != 0 || method.getReturnType() == Void.class) {
          throw new IllegalStateException(
              "Method signature does not match with the GET " + "pattern. Method: " + method);
        }
        type = GET;
      }
      ...
      if (methodName.endsWith(SET_AND_GET.suffix)) {
        if (method.getParameterCount() != 1 || method.getReturnType() == Void.class
            || method.getReturnType() != method.getParameterTypes()[0]) {
          throw new IllegalStateException(
              "Method signature does not match with the SET AND GET " + "pattern. Method: "
                  + method);
        }
        type = SET_AND_GET;
        ...
        if (method.getParameterCount() != 1 || method.getReturnType() == Void.class
          ...
          throw new IllegalStateException(
              "Method signature does not match with the SET AND CHAIN " + "pattern. Method: "
                  + method);
        }
        type = SET_AND_CHAIN;
      } else {
        if (method.getParameterCount() != 1 || method.getReturnType() == Void.class) {
          throw new IllegalStateException(
              "Method signature does not match with the SET " + "pattern. Method: " + method);
        }
        type = SET;
      }
    } else if (methodName.equals(_GET_VIEW_HANDLER.prefix)) {
      type = _GET_VIEW_HANDLER;
    } else if (methodName.equals(HASH_CODE.prefix)) {
      type = HASH_CODE;
    } else if (methodName.equals(EQUALS.prefix)) {
      type = EQUALS;
    } else if (methodName.equals(TO_STRING.prefix)) {
      type = TO_STRING;
    }
  ...
  public String extractFeatureName(Method method) {
    ...
    if (!featureNameLowerCase.startsWith(prefix) || !featureNameLowerCase.endsWith(suffix)) {
      throw new IllegalArgumentException(
          "Extracting feature name from method: " + method + " " + "with RequestType: " + this
